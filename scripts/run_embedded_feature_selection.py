...
def select_top_k_indices(scores: np.ndarray, k: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    top_ranked = ranked_indices[:k]
    selected_for_matrix = np.sort(top_ranked)

    return selected_for_matrix.astype(int), top_ranked.astype(int)
...
def compute_tree_importance_scores(
    ...
    if selector_name == "random_forest_importance":
    ...
    elif selector_name == "extra_trees_importance":
        ...
        raise ValueError(f"Unknown tree selector: {selector_name}")
...
def compute_embedded_scores(
    ...
    if selector_name == "l1_logistic":
        scores = compute_l1_logistic_scores(
            X_train=X_train,
            y_train=y_train,
            random_state=random_state,
        )
    elif selector_name in {"random_forest_importance", "extra_trees_importance"}:
        scores = compute_tree_importance_scores(
        ...
        raise ValueError(f"Unknown embedded selector: {selector_name}")
...
def run_cv_for_dataset_selector(
    dataset: str,
    selector_name: str,
    k_values: List[int],
    models: List[str],
    ...
    for fold in sorted(cv_folds["fold"].unique()):
        ...
        scores, fs_elapsed = compute_embedded_scores(
            selector_name=selector_name,
            X_train=X_train,
            y_train=y_train,
            random_state=random_state,
        ...
        for k in k_values:
            selected_indices, _ = select_top_k_indices(scores, k)
...
def run_final_test_for_dataset_selector(
    dataset: str,
    selector_name: str,
    k_values: List[int],
    models: List[str],
    ...
    for k in k_values:
        selected_indices, ranked_indices = select_top_k_indices(scores, k)
...
def main() -> None:
    ...
    print("Running embedded feature selection experiments.")
    print(f"Datasets: {args.datasets}")
    print(f"Embedded selectors: {args.selectors}")
    ...
    for dataset in args.datasets:
        ...
        if len(valid_k_values) != len(args.k_values):
            ...
            print(f"  skipped k values larger than available features: {skipped}")
        ...
        for selector_name in args.selectors:
            print(f"  Embedded selector: {selector_name}")

            cv_rows = run_cv_for_dataset_selector(
                dataset=dataset,
                selector_name=selector_name,
            ...
            test_rows = run_final_test_for_dataset_selector(
                dataset=dataset,
                selector_name=selector_name,
                k_values=valid_k_values,
                models=args.models,
    ...
    metadata = {
        "experiment_name": "embedded_feature_selection",
        ...
        "models": args.models,
        "notes": [
            "This experiment evaluates embedded feature selection methods.",
            "l1_logistic selects genes using absolute L1 logistic regression coefficients.",
            "random_forest_importance selects genes using Random Forest feature importance.",
            "extra_trees_importance selects genes using Extra Trees feature importance.",
    ...
    print("Embedded feature selection experiment completed.")
