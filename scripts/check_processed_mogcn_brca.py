#!/usr/bin/env python3
"""Validate processed MoGCN BRCA outputs."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check processed MoGCN BRCA dataset")
...
def main() -> None:
    ...
    if X.index.tolist() != y["sample_id"].astype(str).tolist():
    ...
    report = {
        ...
        "label_counts": {str(k): int(v) for k, v in y["label"].value_counts().sort_index().to_dict().items()},
        "class_id_counts": {str(k): int(v) for k, v in y["label_id"].value_counts().sort_index().to_dict().items()},
        "example_stratified_split": {
            "test_size": args.test_size,
            "random_state": args.random_state,
            "train_sample_count": int(len(train_ids)),
            "test_sample_count": int(len(test_ids)),
            "train_label_counts": {str(k): int(v) for k, v in y_train["label"].value_counts().sort_index().to_dict().items()},
            "test_label_counts": {str(k): int(v) for k, v in y_test["label"].value_counts().sort_index().to_dict().items()},
        },
    }
