...
def main() -> None:
    ...
    print(f"Checking filter feature selection results in: {out_dir}")
        ...
        for selector in selectors:
            for k in k_values:
                ...
                if selected["rank"].min() != 1 or selected["rank"].max() != int(k):
                    raise ValueError(f"{selected_path}: rank range is invalid")
    ...
    print("PASS: filter feature selection result files are valid.")
    print("")

    print("Best feature-selection configuration per dataset according to CV Macro F1:")
    cols = [
        "dataset",
        "selector",
        "k",
    ...
    print("Best configuration per dataset and selector:")
    cols2 = [
        "dataset",
        "selector",
        "k",
