...
def main() -> None:
    ...
    print(f"Checking wrapper RFE results in: {out_dir}")
    ...
    for df_name, df in [
        ...
        if (df["n_selected_features"] > df["n_prefilter_features"]).any():
            raise ValueError(f"{df_name}: selected features exceed prefilter features")
    ...
    print("PASS: wrapper RFE result files are valid.")
    print("")

    print("Best wrapper-RFE configuration per dataset according to CV Macro F1:")
    cols = [
        "dataset",
        "selector",
        "k",
    ...
    print("Best wrapper-RFE configuration per dataset and selector:")
    cols2 = [
        "dataset",
        "selector",
        "k",
